...
(function() {
  ...
  function tkHandle(v) { return extractHandle(v, ['tiktok.com']).replace(/^@/, ''); }
  function liHandle(v) {
    if (!v) return '';
    let s = String(v).trim();
    if (!s) return '';
    ...
    if (lower.startsWith('http://') || lower.startsWith('https://') || lower.includes('linkedin.com/')) {
      ...
      try {
        ...
        for (let i = 0; i < segs.length; i++) {
          if (/^(in|company|pub)$/i.test(segs[i]) && segs[i + 1]) return segs[i + 1].replace(/^@/, '');
        }
        if (segs.length) return segs[segs.length - 1].replace(/^@/, '');
      } catch (e) {}
    }
  ...
  function ytHandle(v) {
    ...
    if (lower.startsWith('http://') || lower.startsWith('https://') || lower.includes('youtube.com/') || lower.includes('youtu.be/')) {
      ...
      try {
        ...
        if (/^(channel|c|user)$/i.test(first) && segs[1]) return first.toLowerCase() + '/' + segs[1];
        return '@' + first.replace(/^@/, '');
      } catch (e) {}
    }
    return '@' + s.replace(/^@/, '').replace(/\\/+$/, '');
  }
  function ytLink(v) { return 'https://youtube.com/' + ytHandle(v); }
  function waNumber(v) { return String(v || '').replace(/\\D/g, ''); }

  // ====== Сборка vCard в браузере (с фото) ======
  ...
  function buildVCard() {
    ...
    const fn = [card.first_name, card.last_name].filter(Boolean).join(' ');
    lines.push('N:' + vcardEscape(card.last_name) + ';' + vcardEscape(card.first_name) + ';;;');
    lines.push('FN:' + vcardEscape(fn));
    if (card.company) lines.push('ORG:' + vcardEscape(card.company));
    if (card.position) lines.push('TITLE:' + vcardEscape(card.position));
    if (card.bio) lines.push('NOTE:' + vcardEscape(card.bio));
    if (card.phone) lines.push('TEL;TYPE=CELL,VOICE:' + vcardEscape(card.phone));
    if (card.phone_secondary) lines.push('TEL;TYPE=WORK,VOICE:' + vcardEscape(card.phone_secondary));
    ...
    if (card.website) {
      ...
      lines.push('URL:' + vcardEscape(site));
    }
    if (card.address) lines.push('ADR;TYPE=WORK:;;' + vcardEscape(card.address) + ';;;;');
    if (card.instagram) { const h = igHandle(card.instagram); if (h) lines.push('X-SOCIALPROFILE;TYPE=instagram:https://instagram.com/' + vcardEscape(h)); }
    if (card.telegram) { const h = tgHandle(card.telegram); if (h) lines.push('X-SOCIALPROFILE;TYPE=telegram:https://t.me/' + vcardEscape(h)); }
    if (card.linkedin) { const h = liHandle(card.linkedin); if (h) lines.push('X-SOCIALPROFILE;TYPE=linkedin:https://linkedin.com/in/' + vcardEscape(h)); }
    if (card.whatsapp) { const n = waNumber(card.whatsapp); if (n) lines.push('X-SOCIALPROFILE;TYPE=whatsapp:https://wa.me/' + vcardEscape(n)); }
    if (card.facebook) { const h = fbHandle(card.facebook); if (h) lines.push('X-SOCIALPROFILE;TYPE=facebook:https://facebook.com/' + vcardEscape(h)); }
    if (card.youtube)  { const h = ytHandle(card.youtube);  if (h) lines.push('X-SOCIALPROFILE;TYPE=youtube:https://youtube.com/' + vcardEscape(h)); }
    if (card.tiktok)   { const h = tkHandle(card.tiktok);   if (h) lines.push('X-SOCIALPROFILE;TYPE=tiktok:https://tiktok.com/@' + vcardEscape(h)); }
    if (card.slug) lines.push('URL;TYPE=profile:' + vcardEscape(window.location.origin) + '/' + vcardEscape(card.slug));
    if (card.avatar_url) {
      const m = /^data:image\\/([a-zA-Z]+)(?:;[^,]*)?;base64,(.+)$/.exec(card.avatar_url);
      if (m) {
        let type = m[1];
        if (/^jpe?g$/i.test(type)) type = 'JPEG'; else type = type.toUpperCase();
        const data = m[2].replace(/\\s+/g, '');
        lines.push(vcardFold('PHOTO;ENCODING=b;TYPE=' + type + ':' + data));
      }
    }
  ...
  // ====== Кнопка "Сохранить контакт" — собираем vCard в браузере, мгновенно ======
  // Делаем blob: URL с готовым vCard ещё до клика — телефон сохраняется моментально, без запроса на сервер.
  const saveBtn = document.getElementById('save-contact');
  const serverVcardUrl = '/' + card.slug + '/vcard';
  try {
    const blob = new Blob([buildVCard()], { type: 'text/vcard;charset=utf-8' });
    saveBtn.href = URL.createObjectURL(blob);
  } catch (e) {
    saveBtn.href = serverVcardUrl;
  }
  saveBtn.setAttribute('download', card.slug + '.vcf');

  saveBtn.addEventListener('click', function(e) {
    // iOS PWA в standalone не умеет download — используем серверный endpoint
  ...
  if (card.whatsapp && waNumber(card.whatsapp)) quickActions.push({
    href: 'https://wa.me/' + waNumber(card.whatsapp),
    label: 'WhatsApp',
    icon: '<path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z"/>',
    target: '_blank'
  });
  if (card.telegram && tgHandle(card.telegram)) quickActions.push({
    href: 'https://t.me/' + tgHandle(card.telegram),
    label: 'Telegram',
  ...
  // Соц.сети
  const socials = [];
  if (card.instagram) { const ig = igHandle(card.instagram); if (ig) socials.push({
  ...
  if (card.telegram) { const tg = tgHandle(card.telegram); if (tg) socials.push({
    href: 'https://t.me/' + tg,
    label: 'Telegram', value: '@' + tg,
    icon: '<path d="M22 2L11 13M22 2l-7 20-4-9-9-4 20-7z"/>',
    target: '_blank'
  }); }
  if (card.whatsapp) { const wa = waNumber(card.whatsapp); if (wa) socials.push({
    href: 'https://wa.me/' + wa,
    label: 'WhatsApp', value: card.whatsapp,
    icon: '<path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z"/>',
    target: '_blank'
  }); }
  if (card.linkedin) { const li = liHandle(card.linkedin); if (li) socials.push({
  ...
  if (card.facebook) { const fb = fbHandle(card.facebook); if (fb) socials.push({
    href: 'https://facebook.com/' + fb,
    label: 'Facebook', value: fb,
    icon: '<path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z"/>',
    target: '_blank'
  }); }
  if (card.youtube) { const yt = ytHandle(card.youtube); if (yt) socials.push({
    href: ytLink(card.youtube),
    label: 'YouTube', value: yt.startsWith('@') ? yt : '@' + yt,
  ...
  if (card.tiktok) { const tk = tkHandle(card.tiktok); if (tk) socials.push({
